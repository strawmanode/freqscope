...
/**
 * Handles FreqScope's /api/* routes. Returns true when the request was handled
 * (response ended), or false when the caller should fall through to other
 * handlers (e.g. Vite's next() in dev, or static file serving in production).
 *
 * This is framework-agnostic Node http logic shared between the Vite dev
 * plugin and the standalone production server.
 */
export async function handleApiRequest(
  ...
  try {
    const data = await fetchAircraftInBoundingBox(lamin, lamax, lomin, lomax)
    res.statusCode = 200
    res.setHeader('Content-Type', 'application/json')
